...
class Controller:
    ...
    @classmethod
    def find_candidate_nodes(cls, v_net, p_net, v_node_id, filter=[], check_node_constraint=True, check_link_constraint=False):
        ...
        if check_node_constraint:
            ...
            for p_node_id in checked_nodes:
                check_result = cls.check_node_constraints(v_net, p_net, v_node_id, p_node_id)
                # print(f"check_node_constraints for p: {p_node_id} and v:{v_node_id}, {check_result}")
                if check_result:
    ...
    @classmethod
    def node_mapping(cls, vn, pn, sorted_v_nodes, sorted_p_nodes, solution, reusable=False, inplace=True, matching_method='greedy'):
        """
        matching_method: ['l2s2', 'greedy']
        """
        assert matching_method in ['l2s2', 'greedy']
        ...
        # existing vn not mapped
        if len(solution['node_slots']) < vn.num_nodes:
            solution.update({'place_result': False, 'result': False})
            return False
    ...
    @staticmethod
    def deploy_with_node_slots(vn, pn, node_slots, solution, inplace=True, shortest_method='bfs_shortest', k_shortest=10):
        pn = pn if inplace else copy.deepcopy(pn)
        # unfeasible solution
        ...
        node_mapping_result = Controller.node_mapping(vn, pn, list(node_slots.keys()), list(node_slots.values()), solution, 
                                                            reusable=False, inplace=True, matching_method='l2s2')
