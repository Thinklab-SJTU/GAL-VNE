...
class Environment:
    ...
    def from_config(cls, config):
        ...
        save_dir = config.get('save', 'save/')
        record_dir = os.path.join(save_dir, solver_name, run_id)#, 'records')
        summary_dir = record_dir
        os.makedirs(os.path.join(record_dir, 'models'), exist_ok=True)
        if config.get('save_samples',False):
            os.makedirs(os.path.join(record_dir, 'samples'), exist_ok=True)
        ...
        vn_simulator = VNSimulator.from_setting(config.pop('vns_setting'), is_sub = False, kwargs=config)
    ...
    @classmethod
    def sub_env_from_config(cls, config):
        r"""Create an environment following the settings in config."""
        if not isinstance(config, dict): config = vars(config)
        config = copy.deepcopy(config)
        ...
        pn_dataset_dir = config.get('pn_dataset_dir')
        vns_sub_dataset_dir = config.get('vns_sub_dataset_dir')
        
        pn = PhysicalNetwork.load_dataset(pn_dataset_dir)
        vn_simulator = VNSimulator.from_setting(config.pop('vns_sub_setting'), is_sub = True, kwargs=config)
        recorder = Recorder(summary_dir=summary_dir, save_dir=record_dir, if_temp_save_records=if_temp_save_records)
        return cls(pn, vn_simulator, recorder, verbose, **config)
    ...
    def reset(self, seed=None):
        ...
        if self.recorder.if_temp_save_records and self.verbose >= 2:
        ...
        if self.renew_vn_simulator:
            ...
            self.vn_simulator = self.vn_simulator.load_dataset(self.vns_dataset_dir, self.vn_simulator.is_sub, self.config)
    ...
    def summary_records(self, extra_summary_info={}, summary_file_name=None, record_file_name=None, stage = "infer", epoch_id="", is_verbose=1):
        ...
        if record_file_name is None:
            record_file_name = f'{stage}-EP{epoch_id}-{start_run_time}.csv'
        ...
        if is_verbose >= 1:
            print(f"acc ratio\ttotal_cost\ttotal_revenue\tr/c ratio\tlong_term_avg_revenue\ttotal_time_cost\ttotal_time_revenue\ttime r/c ratio")
            print(f"{summary_info['acceptance_rate']:.4f}\t\t{summary_info['total_cost']}\t\t{summary_info['total_revenue']}\t\t{summary_info['rc_ratio']:.4f}\t\t{summary_info['long_term_aver_revenue']:.4f}\t\t\t{summary_info['total_time_cost']:.2f}\t\t{summary_info['total_time_revenue']:.2f}\t\t{summary_info['time_rc_ratio']:.4f}")
            with open(os.path.join(self.recorder.save_dir, "records", "log.txt"),'a') as f:
                print(f"{stage}-EP{epoch_id}\t{summary_info['acceptance_rate']:.4f}\t{summary_info['total_cost']}\t{summary_info['total_revenue']}\t{summary_info['rc_ratio']:.4f}\t{summary_info['long_term_aver_revenue']:.4f}\t{summary_info['total_time_cost']:.2f}\t{summary_info['total_time_revenue']:.2f}\t{summary_info['time_rc_ratio']:.4f}", file=f)
...
class JointPRStepEnvironment(Environment):
    ...
    def get_observation(self):
        return {'vn': self.curr_vn[self.curr_vnf_id], 'pn': self.pn}
