...
class Recorder:
    ...
    def __init__(self, summary_dir='save/records/', save_dir='save/', if_temp_save_records=True, node_resource_price=0.33, edge_resource_price=1.):
        ...
        os.makedirs(os.path.join(self.save_dir, 'records'), exist_ok=True)
    ...
    def reset(self):
        ...
        if self.if_temp_save_records:
            ...
            while not available_fname:
                temp_save_path = os.path.join(self.save_dir , f'records/temp-{suffixes}.csv')
                suffixes += 1
    ...
    def count_state(cls, vn, pn, solution):
        ...
        # Enter event
            ...
            if solution['result']:
                ...
                vn_id = solution['vn_id']
                for vnf_id, pn_node_id in solution['node_slots'].items():
                    # print("map ", vnf_id, "from :", vn_id, "to :", pn_node_id)
    ...
    def count(cls, vn, pn, solution):
        ...
        # Enter event
            ...
            if solution['result']:
                ...
                solution_info = cls.counter.count_partial_solution(vn, solution)
        else:
            raise NotImplementedError
    ...
    def save_records(self, fname):
        ...
        save_path = os.path.join(self.save_dir, 'records', fname)
...
class Counter(object):
    ...
    def calculate_vn_path_cost(vn, solution=None):
        r"""Calculate the deployment cost of current vn according to `edge paths`."""
        # print("solution['edge_paths']: ", solution['edge_paths'])
        vn_path_cost = 0
        e_attrs = vn.get_edge_attrs('resource')
        for edge, path in solution['edge_paths'].items():
            # print("vn edge: ", edge)
...
class Solution(ClassDict):
    def __init__(self, vn):
        ...
        # print("solution vn id: ", vn.id, "(recorder line 359)")
        self.vn_id = vn.id
    ...
    def reset(self):
        ...
        self.edge_paths = OrderedDict()
        self.node_slots_info = OrderedDict()
        self.edge_paths_info = OrderedDict()
        self.vn_cost = 0
        self.vn_revenue = 0
        self.vn_demand = 0
        self.vn_node_demand = 0
        self.vn_edge_demand = 0
