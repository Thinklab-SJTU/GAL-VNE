...
class RandomizedRoundingSolver(Solver):
    ...
    def __init__(self, **kwargs):
        super(RandomizedRoundingSolver, self).__init__(**kwargs)
        # node mapping
    ...
    def solve(self, instance):
        ...
        candicates_dict = Controller.construct_candidates_dict(v_net, p_net)
    ...
    def deploy_with_v_p_value_dict(self, v_net, p_net, v_p_value_dict, candicates_dict):
        ...
        for v_node_id in list(v_net.nodes):
            ...
            place_and_route_result = Controller.place_and_route(v_net, p_net, v_node_id, p_node_id, solution, 
    ...
    def select_p_net_node(self, v_p_candicate_prob_dict):
        if sum(list(v_p_candicate_prob_dict.values())) == 0:
            return random.choices(list(v_p_candicate_prob_dict.keys()), k=1)[0]
        selected_p_node_id = random.choices(list(v_p_candicate_prob_dict.keys()), weights=list(v_p_candicate_prob_dict.values()), k=1)[0]
    ...
    def construct_resource_dict(self, v_net, p_net, n_attr_name_list, e_attr_name_list, candicates_dict):
        ...
        def get_edge_resource_dict(e_id, e_attr_name):
            ...
            if u_type == 'p' and v_type == 'p':
                if (u, v) in p_net.edges:
                    return p_net.edges[(u, v)][e_attr_name]
                return 0
    ...
    def solve_with_mip(self, v_net, p_net, candicates_dict):
        ...
        for n_id_a in range(num_p_nodes+num_v_nodes):
            for n_id_b in range(num_p_nodes+num_v_nodes):
                for v_edge in v_net.edges:
                    f[(n_id_a, n_id_b, v_edge)] = solver.NumVar(lb=0, ub=self.META_BW, name=f'f({n_id_a, n_id_b, v_edge})')

        # Objective
        # solver.Minimize(
        #     sum(f[(u, v, i)] for i in list(v_net.edges) for u in list(p_net.nodes) for v in list(p_net.nodes))
        #     + \
        #     sum(x[m, w] * node_resource_dict[m][n_attr_name] for m in m_node_list for w in p_node_list for n_attr_name in n_attr_name_list)
        # )

        solver.Minimize(
            sum(1 / (edge_resource_dict[(u,v)][e_attr_name] + 1e-6) * sum(f[(u, v, i)] for i in list(v_net.edges)) for u in list(p_net.nodes) for v in list(p_net.nodes) for e_attr_name in e_attr_name_list)
            + \
            ...
            for m in m_node_list:
                ...
                for p in p_node_list:
                    if p not in candicates_dict[m-num_p_nodes]:
                        ...
                        v_p_value_dict[m-num_p_nodes].append(x[(m,p)].solution_value() * (sum(f[(m, p, i)].solution_value() for i in v_net.edges)) + sum(f[(p, m, i)].solution_value() for i in v_net.edges))
