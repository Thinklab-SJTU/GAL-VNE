...
from ..rank.node_rank import FFDNodeRank, RWNodeRank
...
class JointPRSolver(Solver):
    ...
    def solve(self, instance):
        ...
        for vnf_id in list(vn.nodes):
            ...
            candidate_pn_nodes = Controller.find_candidate_nodes(vn, pn, vnf_id, filter=selected_pn_nodes)
...
class RWJointPRSolver(JointPRSolver):

    def __init__(self, name, reusable=False, verbose=1, **kwargs):
        super(RWJointPRSolver, self).__init__(name='rw_joint_pr', reusable=reusable, verbose=verbose, **kwargs)
