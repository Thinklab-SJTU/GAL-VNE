...
class RolloutBuffer:
    ...
    def compute_direct_returns(self):
        self.returns = self.rewards


    def compute_truncated_returns(self, gamma=0.99):
        discounted_reward = 0
        for reward, is_terminal in zip(reversed(self.rewards), reversed(self.dones)):
            if is_terminal:
                discounted_reward = 0
    ...
    def compute_truncated_values(self, values, gamma=0.99):
        value_returns = []
        discounted_value = 0
        for value, is_terminal in zip(reversed(values), reversed(self.dones)):
            if is_terminal:
                discounted_value = 0
            discounted_value = value + (gamma * discounted_value)
            value_returns.insert(0, discounted_value)
