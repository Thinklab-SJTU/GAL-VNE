...
class State:
    ...
    """
    def __init__(self, p_net, v_net, controller, counter):
    ...
    def compute_final_reward(self):
        ...
        link_result = Controller.link_mapping(self.v_net, self.p_net, solution=solution,
                                                shortest_method='bfs_shortest', k=1, inplace=True)

        if link_result:
            v_net_cost = self.counter.calculate_vn_cost(self.v_net, solution)
            vn_revenue = self.counter.calculate_vn_revenue(self.v_net)
            return 1000 + vn_revenue - v_net_cost
        else:
            return -float('inf')

    def random_select_next_state(self):
        """Random select a physical node to accommodate the next virtual node"""
        candidate_p_nodes = Controller.find_candidate_nodes(
