...
class ObservationHandler:
    ...
    def get_edge_attrs_obs(self, network, link_attr_types=['resource', 'extrema'], normalization=True, benchmark=None):
        e_attrs = network.get_edge_attrs(link_attr_types)
        link_data = np.array(network.get_edge_attrs_data(e_attrs), dtype=np.float32)
    ...
    def get_link_index_obs(self, network):
        link_index = np.array(list(network.edges), dtype=np.int64)
    ...
    def get_v_node_link_demands(self, v_net, v_node_id, normalization=True, benchmark=None):
        e_attrs = v_net.get_edge_attrs('resource')
        link_demands = []
        for l_attr in e_attrs:
            link_demand = [v_net.edges[(n, v_node_id)][l_attr.name] for n in v_net.adj[v_node_id]]
    ...
    def get_v_node_features(self, v_net, v_node_id):
        ...
        for l_attr in v_net.get_edge_attrs('resource'):
            link_demand = [v_net.edges[(n, v_node_id)][l_attr.name] for n in v_net.adj[v_node_id]]
            max_link_demand.append(max(link_demand) / self.link_attrs_benchmark[l_attr.name])
            mean_link_demand.append((sum(link_demand) / len(link_demand)) / self.link_attrs_benchmark[l_attr.name])
