...
class SubEnv(PlaceStepSubRLEnv):
    ...
    def __init__(self, p_net, v_net, **kwargs):
        ...
        num_p_net_link_attrs = len(self.p_net.get_edge_attrs(['resource']))
    ...
    def get_observation(self, p_net):
        p_net_obs = self.get_p_net_obs(p_net)
        return self.obs_for_cnn(p_net_obs)
    

    def get_p_net_obs(self, p_net):
        ...
        # norm_node_attrs_data = np.squeeze(norm_node_attrs_data, axis=-1)
        # sum_bw
        e_attrs = p_net.get_edge_attrs(['resource'])
        ...
        # norm_link_aggr_attrs_data = np.squeeze(norm_link_aggr_attrs_data, axis=-1)
        # avg_dst
        # avg_distance = self.obs_handler.get_average_distance(p_net, self.selected_p_net_nodes, normalization=True)
        p_net_obs = np.concatenate((norm_node_attrs_data, norm_link_aggr_attrs_data, self.p_net_node_degrees), axis=-1)
        return p_net_obs
