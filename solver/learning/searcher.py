...
class SampleSearcher(Searcher):
    ...
    def find_solution(self, sub_env):
        ...
        score_list = [env.solution.vn_rc_ratio if env.solution.result else 0. for env in env_list]
...
class BeamSearcher(Searcher):
    ...
    def find_solution(self, sub_env):
        ...
        while not sum(done_list):
            ...
            if sum(done_list) == self.k:
                break
        score_list = [env.solution.vn_rc_ratio if env.solution.result and env.solution.violation <= 0 else 0. for env in env_list]
