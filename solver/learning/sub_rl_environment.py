...
class SubRLEnv(VNERLEnv):
    ...
    def revoke(self):
        ...
        Controller.undo_place_and_route(self.v_net, self.p_net, last_v_node_id, paired_p_node_id, self.solution)
        solution_info = Counter.count_partial_solution(self.v_net, self.solution)
...
class SolutionStepSubRLEnv(SubRLEnv):
    ...
    def __init__(self, p_net, v_net, **kwargs):
        super(SolutionStepSubRLEnv, self).__init__(p_net, v_net, **kwargs)
    ...
    def get_observation(self):
        return {'vn': self.v_net, 'pn': self.p_net}
...
class NodeSlotsStepSubRLEnv(SubRLEnv):
    ...
    def __init__(self, p_net, v_net, **kwargs):
        super(NodeSlotsStepSubRLEnv, self).__init__(p_net, v_net, **kwargs)
...
class JointPRStepSubRLEnv(SubRLEnv):
    ...
    def __init__(self, p_net, v_net, **kwargs):
        super(JointPRStepSubRLEnv, self).__init__(p_net, v_net, **kwargs)
    ...
    def step(self, action):
        ...
        # Case: reusable = False and place in one same node
            ...
            solution_info = Counter.count_solution(self.v_net, self.solution)
            done = True
            # solution_info = self.solution.to_dict()
        # Case: Try to Place and Route
        else:
            assert p_node_id in list(self.p_net.nodes)
            place_and_route_result = Controller.place_and_route(self.v_net, 
                                                                self.p_net, 
                                                                self.curr_v_node_id, 
                                                                p_node_id,
                                                                self.solution, 
    ...
    def compute_reward(self, solution):
        ...
        if solution['result'] :
            ...
            # node_load_balance = self.get_node_load_balance(self.selected_p_net_nodes[-1])
            # reward = curr_place_progress * (solution['vn_rc_ratio']) #  - 0.01 * node_load_balance
            # reward = curr_place_progress * ((solution['vn_rc_ratio']) + 0.01 * node_load_balance)
            ...
            # reward = weight * solution['vn_rc_ratio']
            ...
            # reward = - 0.1
            # reward = -1. * weight
        # reward = solution['vn_rc_ratio'] if solution['result'] else 0
...
class PlaceStepSubRLEnv(SubRLEnv):
    ...
    def __init__(self, p_net, v_net, **kwargs):
        super(PlaceStepSubRLEnv, self).__init__(p_net, v_net, **kwargs)
    ...
    def step(self, action):
        ...
        # Case: Try to Place
            ...
            node_place_result = Controller.place(self.v_net, self.p_net, self.curr_v_node_id, p_node_id, self.solution)
