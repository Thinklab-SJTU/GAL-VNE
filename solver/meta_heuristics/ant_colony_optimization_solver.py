...
class AntColonyOptimizationSolver(MetaHeuristicSolver):
    ...
    def __init__(self, **kwargs):
        super(AntColonyOptimizationSolver, self).__init__(**kwargs)
        # super parameters
        self.num_ants = 10
    ...
    def calc_heuristic_info(self, p_net):
        ...
        bw_sum_resource = np.array(p_net.get_aggregation_attrs_data(p_net.get_edge_attrs('resource'), aggr='sum', normalized=False)).sum(0).tolist()
    ...
    def evolve(self, ant):
        for v_node_id in ant.v_net.ranked_nodes:
            ...
            if p_node_id != -1:
                result = Controller.place_and_route(ant.v_net, ant.p_net, v_node_id, p_node_id, ant.solution, shortest_method='bfs_shortest', k=1)
            else:
                break
            if not result:
                break
