...
class GRCNodeRank(NodeRank):
    ...
    def rank(self, network, sort=True):
        ...
        def calc_grc_M(network):
            ...
            M = np.array([_.toarray() for _ in M])
            M = np.sum(M,0) 
            # normalize the edge feature matrix
            normalize = np.where(M.sum(axis=0)==0, 1, M.sum(axis=0))  # avoid zero division
            M = M/normalize
        ...
        while(delta >= self.sigma):
            new_node_rank = (1 - self.d) * c + self.d * np.dot(M, node_rank)
...
class RWNodeRank(NodeRank):
    ...
    def rank(self, network, sort=True):
        ...
        def cal_h_u(network):
            ...
            M = sum(M) / len(M)
            bw_data = np.array(M.sum(axis=0)).reshape(network.num_nodes)
        ...
        # print(h_u.shape, adj_matrix.nonzero()[1])
